// page/pack-organizer/sign-appointment/sign-appointment.js
...
Page({
  ...
  onLoad: function (options) {
    that = this;
    mRoleId = '';
    group_sign = options.group_sign || 1;
...
function joinTeam(params) {
  ...
  params.role = mRoleId;
  params.group_sign = group_sign;
  params.user_birth = that.data.teamDate
  httpsUtils.joinOrganization(params, function (res) {
    try {
      var pages = getCurrentPages()
      if (pages.length > 1) {
        typeof (pages[pages.length - 2].freshData(group_sign)) == 'function' && pages[pages.length - 2].freshData(group_sign)
      }
    } catch (e) {
    }
    wx.navigateBack({
    ...
    // var pages = getCurrentPages();
    // pages[pages.length - 2].freshData();//上一页
