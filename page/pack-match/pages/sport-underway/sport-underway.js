...
Page({
  ...
  bindchangeMatchTurns: function (e) {
    ...
    console.log(value, checkLabelIndex, courseLabelItem, matchTurns, courseLabelItem)
    if (mCompetitionLevel == 1) {
      getCompetitionMatch(that)
    } else {
      getLeagueCompetitionMatch(that)
...
function getLeagueCompetitionMatch(that) {
  ...
  params.current_page = courseLabelItem.current_page
  params.page_size = courseLabelItem.page_size
  httpsUtils.leagueMatch(params, function (res) {
    //轮次
...
function getLeagueRankings(that) {
  ...
  httpsUtils.getLeagueRankings(checkLabelIndex3, params, function (res) {
    var httpList = []
    httpList = res.list
    // rankingsLabelItem.total_num = httpList.length
    // if (params.current_page == 1) {
    ...
    if (checkLabelIndex3 == 3 && res.info) {
      that.setData({
        competition_stop_rule: JSON.parse(res.info.stop_rule),
        ['ryBrand1.card_info.yellow_card']: res.stage || `第N轮`
      })
    }
    // } else {
